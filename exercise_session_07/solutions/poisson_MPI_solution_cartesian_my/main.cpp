...
int main(int argc, char *argv[]){
    ...
    create_cartesian(&p,my_rank,indices, &p.left_rank, &p.right_rank, &p.top_rank, &p.bottom_rank);

    // get domain boundaries
    mpi_get_domain(p, my_rank, size, indices, &p.xmin, &p.xmax, &p.ymin, &p.ymax);
