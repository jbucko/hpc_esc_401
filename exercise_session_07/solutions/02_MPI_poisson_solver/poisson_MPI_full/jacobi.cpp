...
double norm_diff(params p, double** mat1, double** mat2){
    ...
    double total_sum;
    ALLREDUCE(&sum,&total_sum, p.comm);
    total_sum /= p.nx*p.ny;
...
void jacobi_step(params p, double** u_new, double** u_old, double** f, int my_rank, int size){
    ...
    halo_comm(p, my_rank, size, u_old, fromLeft, fromRight, fromTop, fromBottom, requests); 
...
void jacobi_step_optimized(params p, double** u_new, double** u_old, double** f, int my_rank, int size){
    ...
    // Inner domain, does not require communication
    ...
    // Outer domain, requires communication
    double top, bottom, left, right;
    for (int i=p.xmin; i<p.xmax; i++){
        if (i==0 || i==p.nx-1) continue;
        for (int j=p.ymin; j<p.ymax; j++){
            ...
            // skip central region
            if (j!=p.ymin && j!=p.ymax-1 && i!=p.xmin && i!=p.xmax-1) continue;
