...
void readParameters(const char* file_name, params *param) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, "domain_decomposition=%d\n", &param->domain_decomposition))
            printf("\t domain_decomposition=%d\n", param->domain_decomposition);
        if (sscanf(line, "communication=%d\n", &param->communication))
            printf("\t communication=%d\n", param->communication);
        if (sscanf(line, "store_rank0=%d\n", &param->store_rank0))
            printf("\t store_rank0=%d\n", param->store_rank0);
    }
    fclose(fp);
}
